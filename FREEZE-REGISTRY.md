...
| `v1.6.2` | 2026-06-11 | **Re-derivação dos checklists de revisão (`canon/20-checklist-revisao.md`).** Os 14 checklists de revisão (caps 01-14) re-derivados top-down como função do conteúdo de cada capítulo (canon+addon+intro), não das user stories. Granularidade consolidada por requisito (~196 → 230 itens). Fecha a cobertura agêntica/IA que faltava (REQ-AGN, ARC-014/015, DEP-011..014, IAC-011/012/013, self-hosted inference, OPS-011..014, US-21, eval suites/TLPT/IAST, prompts-as-code), divide itens compostos em binários, remove itens derivados de US, e corrige defeitos (cap12 `☑`→`☐`, cap14 `draft:true` removido, referências partidas, headers de âmbito, cap03 `sidebar_position`). `make web` limpo (0 broken links novos). Análise bottom-up das lacunas US em `.work-drafts/checklists-v1.6.2-gap-report.md` (reportada, não corrigida). Revisão de `canon/20-checklist`; `canon/25-rastreabilidade` e wave-notes intocados. Autorizado por programme lead. Merge `--no-ff` `02b90395`; commits `2d54b799`, `f9c95e13`. |
