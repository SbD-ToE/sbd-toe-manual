...
| Os gates de segurança são explícitos, binários e bloqueantes antes de ações irreversíveis, com aprovação humana nominal e separação entre sinal automático e decisão? (`CIC-004`/`CIC-011`) | ☐           |
| Cada promoção entre ambientes é atribuível a uma pessoa identificada — quem a decidiu ou, quando a promoção é automática, quem aprovou os critérios — com registo da pessoa (não apenas papel ou conta de serviço), do critério aplicado e da evidência avaliada, sendo os critérios da promoção automática determinísticos, versionados e pré-aprovados, e com o nível de autonomia declarado quando a promoção for executada por um agente, e sem nenhuma promoção decidida por saída de modelo probabilístico? (`CIC-011`) | ☐           |
