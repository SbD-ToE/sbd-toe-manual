...
| CIC-011 | Promoção entre ambientes atribuível a um responsável identificado | - | ✔ | ✔ | Cada promoção entre ambientes é atribuível a uma pessoa identificada: quem a decidiu ou, quando promove automaticamente, quem aprovou os critérios ao abrigo dos quais promove. O registo identifica a pessoa (não apenas papel ou conta de serviço), o critério aplicado e a evidência avaliada. A promoção automática exige critérios **determinísticos, versionados e pré-aprovados**, e o nível de autonomia declarado quando a promoção for executada por um agente; saída de modelo probabilístico não constitui critério de promoção. |
...
- **CIC-011**: não exige presença humana em cada promoção — exige que a promoção seja **atribuível**. Uma pipeline que promove sozinha ao abrigo de critérios determinísticos, versionados e pré-aprovados cumpre o requisito: responde quem aprovou os critérios, e o nível de autonomia (A0–A4) declara o apetite ao risco. O que o requisito exclui é a promoção **sem ninguém a quem imputar** — e a decisão por saída probabilística, que não é critério. Complementa o `CIC-004`: o `004` exige o gate e a aprovação humana nas acções irreversíveis, o `011` exige que fique registado **a quem é imputável**, e estende-o às promoções reversíveis. É o que o `DPL-001` já faz para produção. Em L1, a rastreabilidade de execução do `CIC-005` é considerada suficiente. Quando quem executa a promoção é um agente, aplica-se-lhe adicionalmente a escala A0–A4 e o `REQ-AGN-002`; uma pipeline determinística sem agente não tem nível de autonomia a declarar e cumpre por via dos critérios.
