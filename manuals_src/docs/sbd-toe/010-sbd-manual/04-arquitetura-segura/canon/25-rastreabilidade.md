...
| ACO-SCBI — Supply Chain & Build Integrity | Âncora partilhada para build-platform integrity, protected execution e trusted build-platform choice; não substitui a âncora complementar do Cap. 05 |
...
| ACO-RPR — Release Promotion, Controlled Rollout & Rollback Readiness | Diversificação caveated para minimização de exposição runtime, hardening de protocolos / cabeçalhos, boundary de configuração / serviços e arquitetura de failover / non-persistence; não constitui âncora primária |
| ACO-SLG — Security Event Logging, Audit Trail & Centralized Logging | Diversificação caveated para telemetria de arquitetura, time-source handling, decoy monitoring e relocalização de sensores (`SC-26`, `SC-45*`, `SC-48*`) sem promover o capítulo a âncora primária |
| ACO-TSV — Testing, Security Validation & Empirical Assurance | Diversificação caveated para empirical assurance architecture-heavy, detonation-chamber testing e support de wireless / segmentation test boundaries (`SC-44`, `PCI-11.2*`, `PCI-11.4.5`, `PCI-11.4.7`) sem promover o capítulo a âncora primária |
